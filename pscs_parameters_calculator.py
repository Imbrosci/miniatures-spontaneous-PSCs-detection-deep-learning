...
class ParametersCalculator():
    ...
    def amplitude_stdev(self):
        ...
        for ev in self.x_dp:
            delta_left = min((self.fs / 1000) * 10, ev)
            delta_right = min((self.fs / 1000) * 50,
                              self.signal_lp.shape[0] - (ev + 1))
            temp = np.linspace(ev - delta_left, ev + delta_right - 1,
                               int(delta_left + delta_right)).astype('int')
        ...
        stdev = np.zeros((len(self.x_dp)))

        for ev_idx in range(len(self.x_dp)):
            delta_left = min(self.fs, self.x_dp[ev_idx])
            delta_right = min(self.fs,
                              signal_wo_events.shape[0] - self.x_dp[ev_idx])
            ...
            stdev[ev_idx] = np.nanstd(signal_wo_events[
                self.x_dp[ev_idx] - delta_left:
                    self.x_dp[ev_idx] + delta_right])
    ...
    def decay_time(self):
        ...
        for i in range(len(x_dp)):
            ...
            delta_right = min(steps, self.signal_lp.shape[0] - (x_dp[i]))
