    ...
    for ch, signal in data.items():
            ...
            if (current_pred >= thr1) and (p >= t):
                ...
                for peak_x in find_peaks(iceberg_pred)[0]:
                    ...
                    if peak_x == biggest_peak:
                        ...
                        delta_left = min(four_ms_dp, uncorrect_peak_x)
                        delta_right = min(
                            four_ms_dp, signal_lp.shape[0] - uncorrect_peak_x)
                        ...
                        if correct_peak_x not in x_dp_pre:
        ...
        for i in range(len(x_dp_pre)):
            ...
            if data_win_300.shape[0] == 300:
                pred = model_refinement.predict(data_win_300.reshape(1, -1, 1))
                if pred >= 0.45:
                    x.append(x_pre[i])
        ...
        try:
            ...
            main_result_sheet.write(mrs_current_row, 7, 'no event found')
        try:
            main_result_sheet.write(mrs_current_row, 8, average_amplitude)
        except TypeError:
            main_result_sheet.write(mrs_current_row, 8, 'no event found')
        ...
        try:
            ...
            main_result_sheet.write(mrs_current_row, 9, 'no event found')
        try:
            main_result_sheet.write(mrs_current_row, 10, mean_decay_time)
        except TypeError:
            main_result_sheet.write(mrs_current_row, 10, 'no event found')
        try:
            main_result_sheet.write(mrs_current_row, 11, average_stdev)
        except TypeError:
            main_result_sheet.write(mrs_current_row, 11, 'no event found')
