...
for index, row in metadata.iterrows():
    ...
    # get information from the metadata.xlsx file
    ...
    for col in range(len(row)):
        if col < 2:
            ...
            print(row.index[col][:-5] + ':', int(row[col]), row.index[col][-3:-1])
        elif col == 5:
            print(row.index[col][:-5] + ':', int(row[col]), row.index[col][-3:-1])
        else:
            print(row.index[col][:-6] + ':', int(row[col]), row.index[col][-4:-1])
    ...
    for ch, signal in data.items():
        ...
        shift = [0, 100, 200]
        prediction_trace = np.zeros((signal_z_scored.shape[0]))
        ...
        for idx in range(len(shift)):
            ...
            prediction_traces[:, idx] = pred.reshape(-1,)
